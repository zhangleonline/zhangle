...
class ProcessData(object):
    ...
    def get_data_variable(self, temp_line):
        ...
        dic1 = {'GPS,': fun1, 'IMU,': fun2, 'MAG,': fun3, 'IMU2': fun4, 'IMU3': fun5, 'MAG2': fun6, 'MAG3': fun7,
                'CTUN': fun8, 'ACI,': fun9, 'MKF1': fun10, 'RTK,': fun11}
        for x in temp_line:
            tmpx = x[0:4]
            if dic1.get(tmpx):
                dic1.get(tmpx)(x)
    ...
    def handle_one_log(self, root, x):
        ...
        dic = self.get_data_variable(temp_line)
        list_IMU_AccZ = np.array(dic['list_IMU_AccZ'])
        list_GPS_TimeUS = np.array(dic['list_GPS_TimeUS'])  # 创建数组
        list_GPS_NSats = np.array(dic['list_GPS_NSats'])
        list_IMU_GyrZ_diff = np.diff(np.array(dic['list_IMU_GyrZ']))  # diff是每行后面一个数减前面一个数
        list_IMU_AccZ_diff = np.diff(np.array(dic['list_IMU_AccZ']))
        list_MAG_MagZ_diff = np.diff(np.array(dic['list_MAG_MagZ']))
        list_IMU2_GyrZ_diff = np.diff(np.array(dic['list_IMU2_GyrZ']))
        list_IMU2_AccZ_diff = np.diff(np.array(dic['list_IMU2_AccZ']))
        list_IMU3_GyrZ_diff = np.diff(np.array(dic['list_IMU3_GyrZ']))
        list_IMU3_AccZ_diff = np.diff(np.array(dic['list_IMU3_AccZ']))
        list_MAG2_MagZ_diff = np.diff(np.array(dic['list_MAG2_MagZ']))
        list_MAG3_MagZ_diff = np.diff(np.array(dic['list_MAG3_MagZ']))
        list_MAG_LENGTH = np.sqrt(np.square(np.array(dic['list_MAG_MagZ'])) + np.square(np.array(dic['list_MAG_MagX'])) + np.square(np.array(dic['list_MAG_MagY'])))  # 地磁模长

        list_CTUN_A = np.array(dic['list_CTUN_A'])
        list_CTUN_DA = np.array(dic['list_CTUN_DA'])
        list_CTUN_THO = np.array(dic['list_CTUN_THO'])

        if len(dic['list_ACI_T4']) == 0:
            ACI_T4 = '未记录'
        else:
            ACI_T4 = dic['list_ACI_T4'][-1]  # 只需要一个字符就行

        if len(dic['list_ACI_T5']) == 0:
            ACI_T5 = '未记录'
        else:
            ACI_T5 = dic['list_ACI_T5'][-1]  # 只需要一个字符就行

        list_MKF1_ARD = np.array(dic['list_MKF1_ARD'])
        list_MKF1_TIMEUS = np.array(dic['list_MKF1_TIMEUS'])
        list_RTK_SATS = np.array(dic['list_RTK_SATS'])
        ...
        return (list_GPS_TimeUS, list_GPS_NSats, list_IMU_GyrZ_diff, list_IMU_AccZ_diff, list_MAG_MagZ_diff,
                ...
                list_MAG3_MagZ_diff, list_CTUN_A, list_CTUN_DA, list_CTUN_THO, ACI_T4, ACI_T5, list_MKF1_ARD,
                list_MKF1_TIMEUS, list_RTK_SATS, list_IMU_AccZ, list_MAG_LENGTH)
    ...
    def judgeSensor_and_print(self,pd_statistical, x):
        ...
        flag_imu_gyro = self.judge_Continuous_zero(self.list_IMU_GyrZ_diff, 30)  #字典当中的键值还是多个列表组成的字典
